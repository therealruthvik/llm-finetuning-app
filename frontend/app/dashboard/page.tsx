      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/new-job" className="text-indigo-400 hover:text-indigo-300 text-sm">
              + New job →
            </Link>
          </div>
                ...
                <div key={ds.id} className={`flex items-center justify-between px-4 py-3 ${i > 0 ? "border-t border-gray-800" : ""}`}>
                  <div>
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">
                      ...
                      {ds.file_size_bytes ? `${(ds.file_size_bytes / 1024).toFixed(1)} KB` : ""}
                    </div>
                  </div>
                  <button
    ...
    <Link href={`/jobs/${job.id}`}>
      <div className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex items-center justify-between hover:border-gray-600 transition-colors cursor-pointer">
        ...
        <div className="text-right">
          {job.final_loss && (
            <div className="text-xs text-gray-300">loss {job.final_loss.toFixed(4)}</div>
          )}
          {job.training_time_s && (
            <div className="text-xs text-gray-500">
              {Math.floor(job.training_time_s / 60)}m
            </div>
          )}
