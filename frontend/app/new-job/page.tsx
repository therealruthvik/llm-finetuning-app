...
const MODELS = [
  { id: "unsloth/Llama-3.2-3B-Instruct", label: "Llama 3.2 3B (Recommended for T4)" },
  ...
  { id: "unsloth/mistral-7b-instruct-v0.3", label: "Mistral 7B (Needs more VRAM)" },
];
          ...
          {["dataset", "config", "hf"].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              ...
              <span className={step === s ? "text-indigo-400 font-medium" : "text-gray-500"}>
                {i + 1}. {s === "dataset" ? "Dataset" : s === "config" ? "Training Config" : "HuggingFace"}
              </span>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-gray-400 text-sm">
                JSON format: list of objects with <code className="text-indigo-300">instruction</code> and <code className="text-indigo-300">output</code> fields.
              </p>
            </div>
            ...
            <div className="border-2 border-dashed border-gray-700 rounded-xl p-6 text-center hover:border-indigo-500 transition-colors">
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                ...
                <div className="text-xs text-gray-500 mt-1">Max 50MB · instruction + output columns required</div>
              </label>
            </div>
        ...
        {step === "config" && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">
                  Epochs <span className="text-gray-500">(1–5)</span>
                </label>
                <input
                  type="number"
                  min={1} max={5}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">
                  LoRA Rank <span className="text-gray-500">(r)</span>
                </label>
                <select
                  value={loraR}
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg px-4 py-3 text-blue-300 text-sm">
              💡 Estimated GPU: T4 (Modal free tier). 3B model ~30–60 min per epoch.
            </div>

            <div className="flex gap-3">
              <button onClick={() => setStep("dataset")} className="flex-1 border border-gray-700 text-gray-300 font-medium py-2 rounded-lg text-sm hover:border-gray-500 transition-colors">
                ← Back
              </button>
              <button onClick={() => setStep("hf")} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-medium py-2 rounded-lg text-sm transition-colors">
                Continue →
              </button>
            </div>
          </div>
        ...
        {step === "hf" && (
          <div className="space-y-5">
            <div>
              ...
              <p className="text-gray-400 text-sm">
                Trained model will be pushed to your HuggingFace Hub repo.
              </p>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">
                HF Token <span className="text-red-400">*</span>
              </label>
              <input
                type="password"
                value={hfToken}
            ...
            {hfUsername && hfRepoName && (
              <div className="text-xs text-gray-400 bg-gray-900 border border-gray-800 rounded-lg px-3 py-2">
                Model will be pushed to:{" "}
                <span className="text-indigo-300 font-mono">{hfUsername}/{hfRepoName}</span>
              </div>
            )}

            {/* Summary */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-2 text-sm">
              <div className="font-medium text-gray-300 mb-2">Job Summary</div>
              {[
                ["Model", baseModel.split("/").pop()],
                ["Epochs", epochs],
                ["LoRA r", loraR],
                ["Learning rate", learningRate],
                ["GPU", "T4 (Modal free tier)"],
              ].map(([k, v]) => (
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep("config")} className="flex-1 border border-gray-700 text-gray-300 font-medium py-2 rounded-lg text-sm hover:border-gray-500 transition-colors">
                ← Back
              </button>
              <button
                onClick={handleSubmit}
                disabled={submitting || !hfToken || !hfUsername}
