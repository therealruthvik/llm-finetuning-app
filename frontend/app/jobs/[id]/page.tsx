    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-center">
          <div className="text-gray-400 mb-2">Job not found</div>
          <Link href="/dashboard" className="text-indigo-400 text-sm">← Dashboard</Link>
        </div>
      </div>
    );
  }
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          ...
          {job.error_message && (
            <div className="mt-4 bg-red-500/10 border border-red-500/30 rounded-lg px-4 py-3 text-red-400 text-sm font-mono">
              {job.error_message}
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: "Batch Size", value: job.batch_size ?? "—" },
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            ...
            <div className="flex items-end gap-0.5 h-24">
              {lossData.map((l, i) => {
                ...
                const pct = ((l.loss! - minLoss) / range);
                const height = Math.max(4, (1 - pct) * 100);  // invert — lower loss = taller bar
                return (
                  <div
                    key={i}
            ...
            {job.logs.length === 0 ? (
              ...
              job.logs.map((log, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <span className={log.loss != null ? "text-green-400" : "text-gray-300"}>
                    {log.log_line}
                  </span>
                </div>
              ))
